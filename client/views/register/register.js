...
Template.Register.events({
	'submit #register_form' : function(e, t) {
		...
		if(register_name == "")
		{
			pageSession.set("errorMessage", "Bitte Namen eingeben");
		...
		{
			pageSession.set("errorMessage", "Bitte gültige Email verwenden.");
		...
		{
			pageSession.set("errorMessage", "Das Passwort muss mindestens " + min_password_len + " Zeichen enthalten.");
		...
		Accounts.createUser({email: register_email, password : register_password, profile: { name: register_name }}, function(err) {
			...
			if(err) {
				if(err.error === 499) {
					...
					pageSession.set("errorMessage", "Bitte warten, Zugang muss bestätigt werden")
				} else
				{
